...
left.preview.link(xoutLeft.input)
if enableRectified:
...
with dai.Device(pipeline) as device:
    while not device.isClosed():
        ...
        for q in queueNames:
            ...
            if q == "rgb":
                cv2.imshow("rgb", message.getCvFrame())
            ...
            if type(message) == dai.ImgFrame:
                ...
                if 'disparity' in q:
                    ...
                    # yuv = frame.reshape(shape).astype(np.unint8)
                    # bgr = cv2.cvtColor(frame, cv2.COLOR_YUV2BGR_IYUV)
                    cv2.imshow(q, frame)
